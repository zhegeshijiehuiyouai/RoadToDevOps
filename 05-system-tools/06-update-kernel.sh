...
KERNEL_VERSION="$(curl -s --connect-timeout 5 https://elrepo.org/linux/kernel/el7/x86_64/RPMS/ | grep "kernel-lt-[0-9].*" | awk -F 'href="' '{print $2}' | awk -F '"' '{print $1}' | tail -1 |awk -F "-" '{print $3}')-1"
