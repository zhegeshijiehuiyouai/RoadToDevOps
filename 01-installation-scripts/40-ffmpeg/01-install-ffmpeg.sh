...
echo_info 安装依赖
yum install -y gcc yasm

echo_info 下载ffmpeg源码包
# 2.4.11之前的老版本没有tar.xz的包，但是这里默认不会下载这么老的版本，所以直接写死了
download_tar_gz ${download_dir} http://www.ffmpeg.org/releases/ffmpeg-${ffmpeg_version}.tar.xz

cd ${file_in_the_dir}
echo_info 解压压缩包
tar xf ffmpeg-${ffmpeg_version}.tar.xz
cd ffmpeg-${ffmpeg_version}
echo_info 编译ffmpeg中，请耐心等待
...
echo_info 配置环境变量
echo "export PATH=$PATH:/usr/local/ffmpeg/bin" > /etc/profile.d/ffmpeg.sh
source /etc/profile
echo_info ffmepg已安装完毕，版本：
ffmpeg -version

echo_warning 由于bash特性限制，在本终端执行ffmpeg命令需要先手动执行 source /etc/profile 加载环境变量，或者重新打开一个终端
