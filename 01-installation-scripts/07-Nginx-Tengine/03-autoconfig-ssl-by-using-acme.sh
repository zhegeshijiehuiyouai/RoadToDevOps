...
FINAL_CONF="${NGINX_CONF_DIR}/${DOMAIN//\*/_}.conf"
...
insert_challenge_location() {
    ...
    sed -i "/server_name.*$DOMAIN/a\    location /.well-known/acme-challenge/ { root $WEBROOT; }" "$f"
...
patch_existing_conf() {
    ...
    if ! grep -Poz "(?s)server\s*{[^}]*listen[^;]*\b80\b[^}]*server_name[^;]*\b$DOMAIN\b" "$file" &>/dev/null; then
...
EXIST_CONF=$(grep -Rsl "server_name.*\<${DOMAIN}\>" "$NGINX_CONF_DIR" /etc/nginx/sites-* 2>/dev/null || true)
...
reload_nginx
echo_info "OK! 现在可以访问: https://${DOMAIN#\*.}"
